...
% we will see the effect of varying lambda on this task with ]dutch
% traces
alpha=0.1;
    ...
    [~,curact]=max(qvalues(csi,:)); % the greedy action corresponding to start state
     ...
     if (rand(1)< epsilon)  % exploratory action.
        ...
        while (terminate~=1)
          ...
          eltrace(csi,curact)=(1-alpha)*eltrace(csi,curact)+1; %Dutch traces
...
xlabel('Number of episodes elapsed'),ylabel('Steps per episode'),title('Agent with Dutch traces on Maze task');
